...
const RopeSnap = (props: Props) => {
  ...
  const animatedProps = useAnimatedProps(() => {
    const p1 = {
      x: Math.floor(x1.value + CENTER_CIRCLE),
      y: Math.floor(y1.value + CENTER_CIRCLE),
    };
    const p4 = {
      x: Math.floor(x2.value + CENTER_CIRCLE),
      y: Math.floor(y2.value + CENTER_CIRCLE),
    };

    const path = createPath(p1);
    addCurve(path, {
      c1: {x: Math.floor(xSPring.value), y: Math.floor(ySPring.value)},
      c2: {x: Math.floor(x2SPring.value), y: Math.floor(y2SPring.value)},
      to: p4,
    });
    return {
      d: serialize(path),
      stroke: {type: 0, payload: pugStokeColor.value},
    };
  });
  const animatedShadowProps = useAnimatedProps(() => {
    const p1 = {
      x: Math.floor(x1.value + CENTER_CIRCLE),
      y: Math.floor(y1.value + 10 + CENTER_CIRCLE),
    };
    const p4 = {
      x: Math.floor(x2.value + CENTER_CIRCLE),
      y: Math.floor(y2.value + 10 + CENTER_CIRCLE),
    };

    const pathShadow = createPath(p1);
    addCurve(pathShadow, {
      c1: {x: Math.floor(xSPring.value), y: Math.floor(ySPring.value + 12)},
      c2: {x: Math.floor(x2SPring.value), y: Math.floor(y2SPring.value + 12)},
      to: p4,
    });
